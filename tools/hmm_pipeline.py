...
options = {
    ...
    "sam": {
        "build": {},
        "search": {"-select_mdalign": 4, "-select_seq": 4, "-select_align": 4, "-select_score": 8, "-sort": 4,
                   "-Emax": 10, "-mdEmax": 1}
    }
}
...
class HMM(object):
    def __init__(self, name, reference_data, model_file=None, path=""):
        ...
        if self.model_file is None:
            if name.endswith("hmmer"):
                ...
                process = Popen(
                    [os.path.join(self.path, "modelfromalign"), self.name, "-alignfile", self.reference_data, *options])
            else:
    ...
    def search(search_data):
        ...
        if self.name.endswith("hmmer"):
            ...
            process = Popen(
                [os.path.join(self.path, "hmmsearch"), *options, "-o", self.results_file, self.model_file, search_data])
        elif self.name.endswith("sam"):
            # $(SAMSCORE) $@ -i $^ -db $(APIDB) $(SAMOPTS)
            self.results_file = "{}_{}.out"
            options = [str(item) for opt in options["hmmer"]["search"].items() for item in opt]
            process = Popen(
                [os.path.join(self.path, "hmmsearch"), self.name, "-i", self.model_file, "-db", search_data, *options])
